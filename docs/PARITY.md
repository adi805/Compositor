...
| LayerTransform.swift | 235 | partial | WS5: Scaled/Rounded/Mirrored/IsValid + typed scale/rotate UI. WS11: display transform dihormati layar + export (LayerPlacement/LayerGeometry). Gap: alat gambar masih nulis di koordinat kanvas (clip seleksi juga), drag handles interaktif belum |
...
- Sisa yang BELUM dari transform: (1) flip tetap di-bake dan flag-nya sengaja DIABAIKAN renderer - v0.1/v0.2 nyimpen pixel bake + flag, jadi menghormati flag tanpa migrasi bikin layer lama ke-flip dobel; ini keputusan kompatibilitas `.comp`, bukan bug kecil. (2) Alat gambar (kuas, blur, smudge, clone, gradient, shape) masih nulis di koordinat kanvas dan clip seleksi juga koordinat kanvas, jadi nggeser layer lalu ngecat di situ bikin stroke nyimpang. Perbaikannya = masuk ke layer space + ikutan nge-transform clip seleksi, satu increment sendiri. (3) `TransformOverlay` (handle drag) belum ada, itu WS11 yang paling keliatan.
