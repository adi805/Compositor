...
| ImageExporter.swift | 146 | partial | WS10: PNG + JPEG export, cap 30k/side + 100MP, DPI (pHYs / JFIF), matte untuk transparency. Gap: composit ignore Layer.Transform (layar JUGA ignore, jadi belum beda layar vs export tapi dua-duanya gak honor scale/rotate), adjustment/folder masks belum; transform-driven rendering + migrasi flip = WS11 |
...
- DEFECT yang ketahuan dan sengaja belum dibenerin di sini: `Flatten.ToRgba` dan `SurfaceOps.CompositeStack` ignore `Layer.Transform`, dan `CanvasView` juga ignore buat layer berpixel (dia gambar ke `canvasRect` penuh; `Scaled(...)` cuma kepake buat placeholder layer kosong). Jadi scale/rotate yang diiket lewat UI WS5 gak ngaruh di mana-mana, dan flip keliatan bener cuma karena v0.2 bake pixel. Bikin rendering transform-aware = WS11 dan butuh keputusan kompatibilitas file dulu: `.comp` v0.1/v0.2 nyimpen pixel yang udah ke-bake DAN flag flip-nya, jadi tanpa migrasi layer lama bakal ke-flip dobel.
