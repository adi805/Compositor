...
- TIFF and HEIC import are refused, and that refusal is now measured instead of assumed: the shipped SkiaSharp 2.88.9 build returns no codec for either container, tested against real files written by Pillow 12.3.0, LIBTIFF 4.5.1 and the libheif bundled in pillow_heif 1.8.0 (`CodecProbeTests`; numbers in `docs/PARITY.md` WS21). Upstream reads both through ImageIO for free. HEIC needs an HEVC decoder that is not present in this stack at all, so it is not a gate we can simply open.
